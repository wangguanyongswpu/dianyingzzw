...
if (getCookie("stime") == null) {
    ...
    var stime = tmp.substr(0,10);
    setCookie("stime",  stime, "s300", cookiedomain);
}
...
if (/play/i.test(location.pathname)) {
    ...
    if ($("header h1").text(vtitle), $("#playerwrap").css({
        ...
        $("#playTip2").html('非会员只能试看20秒，<a href="'+"http://" + rhost + "/?d=" + window.btoa(refid) + "&do=check.html" +'">成为会员</a>观看全部').on("click",
          function() {
              window.location.href = "http://" + rhost + "/?d=" + window.btoa(refid) + "&do=check.html";
...
if (/check/i.test(location.pathname)) {
  ...
  var paylink=apiurl+"?"+baseQs
    + "&ruel=" + location.href + "&money="+money+'&'+baseQs + "&time=" + getCookie("stime");
  if(uhost){
